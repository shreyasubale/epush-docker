...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		getColumnLabelArrayData = function() {
		    var p = ts.p, objColModel = p.colModel;
            var arrData = [];
            var nDataIndex = 0;
            for (var nColumnIndex =1;nColumnIndex < objColModel.length; nColumnIndex++)
            {
                arrData [nDataIndex] = objColModel [nColumnIndex].name;
                nDataIndex ++;
            }
		...
		refreshIndex = function() {
			...
			ts.p._index = [];
			var arrColumnLabels = getColumnLabelArrayData ();
			for(i =0;i < datalen; i++)
			{
                if (typeof ts.p.data[i] === 'string')
                {
                    var strInfoText = ts.p.data[i];
                    var arrInfo = (strInfoText + "").split(DATA_SEPERATOR);

                    var arrData = {};
                    for(var nColumnIndex =0;nColumnIndex < arrColumnLabels.length; nColumnIndex++)
                    {
                        arrData[arrColumnLabels [nColumnIndex]] = arrInfo [nColumnIndex];
                    }
                    ts.p.data[i] = arrData;
                }
